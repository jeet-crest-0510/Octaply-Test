...
async def find_code_from_email(email_text: str):
    code_match = re.search(r"(?i)verification code\s*([\d]{6})", email_text)
    return code_match.group(1) if code_match else None


async def get_verification_code(target_email: str) -> str | None:
    ...
    for item in items:
        ...
        code = await find_code_from_email(body)
...
async def check_verification_form(driver):
    input_element = driver.find_element(By.ID, "input-passcode")
    return input_element.is_displayed()


async def fill_verification_form(driver, code):
    input_element = driver.find_element(By.ID, "input-passcode")
    input_element.send_keys(code)


async def check_invalid_code(driver):
    try:
...
#     code = await get_verification_code("jeannierose@octa-jobs.com") #For testing purposes
#     print(code)